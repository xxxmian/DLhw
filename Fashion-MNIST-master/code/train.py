...
if __name__ == "__main__":
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(n_epoch):
            ...
            print ("[*] Epoch %d/%d, Training start..." % (epoch+1, n_epoch))
            mean_train_acc = 0
            mean_train_loss = 0
            mean_train_time = 0
            ...
            print ("\n[*] Epoch %d/%d, Training done!\n\tloss = %.3e, acc = %.3f, time = %.3f" \
                   % (epoch+1, n_epoch, mean_train_loss, mean_train_acc, mean_train_time))
                
            print ("[*] Epoch %d/%d, Validation start..." % (epoch+1, n_epoch))
            mean_valid_acc = 0
            mean_valid_loss = 0
            mean_valid_time = 0
            ...
            print ("[*] Epoch %d/%d, Validation done!\n\tloss = %.3e, acc = %.3f, time = %.3f" \
                   % (epoch+1, n_epoch, mean_valid_loss, mean_valid_acc, mean_valid_time))
            ...
            if if_test:
                print ("[*] Epoch %d/%d, Testing start..." % (epoch+1, n_epoch))
                mean_test_acc = 0
                mean_test_loss = 0
                mean_test_time = 0
                ...
                print ("[*] Epoch %d/%d, Testing done!\n\tloss = %.3e, acc = %.3f, time = %.3f" \
                       % (epoch+1, n_epoch, mean_test_loss, mean_test_acc, mean_test_time))
            else:
                print ("[*] Epoch %d/%d, No testing!" % (epoch+1, n_epoch))
